...
@app.delete("/jobs/{job_id}/webhook")
async def remove_webhook(job_id: str):
    """Remove completion webhook"""
    # Update job data
    if redis_client:
        job_data = redis_client.get(f"job:{job_id}")
        if job_data:
            job_data = json.loads(job_data)
            job_data["webhook_url"] = None
            redis_client.set(f"job:{job_id}", json.dumps(job_data))
    else:
        if job_id in jobs_db:
            jobs_db[job_id]["webhook_url"] = None
        else:
            raise HTTPException(status_code=404, detail="Job not found")
    
    return {"job_id": job_id, "webhook_removed": True}
